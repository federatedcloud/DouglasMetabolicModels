...
options.rxnNameList = model.rxns;

% determine optimum
[solSC, resSC] = runSteadyCom(model, options);
ind = find(model.c);

[minFlux, maxFlux, minFD, maxFD] = runSteadyComFVA(model, options);
...
csense(ind) = 'G';
% model.lb(ind) = 0.999 * resSC.flux(ind);
model.lb(ind) = min([minFD maxFD]');
